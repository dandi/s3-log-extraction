...
- Consolidated the agent instructions into `AGENTS.md`, which is now the only place they live. The separate GitHub Copilot instructions duplicated some rules, contradicted none, and opened by telling the reader to consult a setup file that has never existed in this repository. Their substance moves across: NumPy-style docstrings and type hints, preferring the environment that is already installed, keeping commits small, and the terms on which CI workflows may be edited. `.github/copilot-instructions.md` stays as a pointer, since tooling looks for it by path, but carries no rules of its own. Recorded at the same time is a style preference that had never been written down: a result is bound to a named variable before being returned rather than returned as an expression, because the name records how to read the value and leaves somewhere to set a breakpoint. `ruff` enforces the opposite through `RET504`, which is now listed under `ignore` with a comment explaining itself. That entry is dormant, since the `RET` rule set is not selected, but `fixable` is set to everything, so selecting `RET` later would otherwise let a single `ruff --fix` collapse every named return at once. ([#302](https://github.com/dandi/s3-log-extraction/pull/302))
