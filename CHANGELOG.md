...
- Added a `number_of_views` column to the per-dataset `by_asset.tsv` summaries. A view is a streaming session rather than a request, defined as a maximal run of streaming (HTTP 206) requests from one IP address to one asset in which no two consecutive requests are more than 8 hours apart. Full downloads (HTTP 200) are never views and continue to be reported by `number_of_downloads`. The threshold is the `SESSION_TIMEOUT_SECONDS` constant. Each dataset also gains a `view_count.tsv` holding its total, which is aggregated into `archive/view_count.tsv` and reported as `total_number_of_views` in `totals.json` and `archive_totals.json`. ([#293](https://github.com/dandi/s3-log-extraction/pull/293))
- Replaced the modulo rounding of the per-asset summaries with a release gate on requester diversity. Every metric of an asset (`bytes_sent`, `number_of_requests`, `number_of_downloads`, and `number_of_views`) is now released exactly, but only when more than `MINIMUM_REGIONS_FOR_DISCLOSURE` genuine geographic regions accessed that asset during the reporting week. Cloud service, VPN, and unresolved region labels do not count toward the threshold. An asset that does not qualify keeps the values of its previous release verbatim, so that consecutive releases cannot be subtracted to expose the few requesters seen in between. An asset that has never qualified is published with conservatively rounded values instead. The exact counts always remain in the extraction cache regardless of what is published. ([#293](https://github.com/dandi/s3-log-extraction/pull/293))
- Limited `generate_summaries` to one release per `SUMMARY_UPDATE_INTERVAL_DAYS`. Calling it again inside that interval raises a `RuntimeError` naming the earliest permitted date rather than republishing. The release moment is recorded in `summaries/summary_update_state.json` after a successful pass, and a `reference_datetime` argument controls which week is being reported. ([#293](https://github.com/dandi/s3-log-extraction/pull/293))

### 🏠 Internal

- Added a `summarize` globals module holding `SESSION_TIMEOUT_SECONDS`, `SUMMARY_UPDATE_INTERVAL_DAYS`, `MINIMUM_REGIONS_FOR_DISCLOSURE`, the request timestamp format, and the summary column groupings. ([#293](https://github.com/dandi/s3-log-extraction/pull/293))
- Reduced the per-asset summaries to a single pass over each asset's per-request files, so `ips.txt` is decrypted once per asset for both the view counts and the region gate. ([#293](https://github.com/dandi/s3-log-extraction/pull/293))
