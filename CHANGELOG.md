...
- Summary generation now raises a `RuntimeError` when an asset's `timestamps.txt`, `download.txt`, and `ips.txt` are not line-aligned, instead of counting that asset as having no views. Line N of each file must describe the same request. A short `download.txt` means the asset was extracted before that file existed, so the extraction cache is incompatible and the asset must be re-extracted. Any other mismatch means the extraction cache is corrupted, most often by an extraction interrupted partway through writing these files. ([#293](https://github.com/dandi/s3-log-extraction/pull/293))
