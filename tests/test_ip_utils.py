...
def test_ip_utils(tmpdir: py.path.local, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    s3_log_extraction.ip_utils.update_ip_to_region_codes(cache_directory=test_cache, encrypt=False)
    s3_log_extraction.ip_utils.update_region_code_coordinates(cache_directory=test_cache, encrypt=False)
