...
from s3_log_extraction.utils import get_ip_stats
...
def test_encrypted_extraction_does_not_store_plaintext_ips(tmp_path: pathlib.Path) -> None:
    ...
    decrypted_ips = set(read_extracted_ips(extractor.extraction_directory, use_encryption=True))
...
def test_encrypted_extraction_accumulates_across_calls(tmp_path: pathlib.Path) -> None:
    ...
    incremental_ips = read_extracted_ips(incremental_extractor.extraction_directory, use_encryption=True)
    single_pass_ips = read_extracted_ips(single_pass_extractor.extraction_directory, use_encryption=True)
...
def test_ip_stats_over_encrypted_extraction(
    ...
    assert ip_stats["extracted_ip_count"] == len(
        set(read_extracted_ips(extractor.extraction_directory, use_encryption=True))
    )
