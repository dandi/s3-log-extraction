...
class DandiS3LogAccessExtractor(S3LogAccessExtractor):
    ...
    def __init__(self, cache_directory: pathlib.Path | None = None) -> None:
        ...
        awk_filename = "_fast_dandi_extraction.awk" if sys.platform != "win32" else "_fast_dandi_extraction_windows.awk"
        ...
        self.awk_base = "gawk" if self.awk_base == "awk" else self.awk_base.parent / "gawk.exe"
    ...
    def _run_extraction(self, *, file_path: pathlib.Path) -> None:
        ...
        self._awk_env["TEMPORARY_DIRECTORY"] = str(self.extraction_directory.absolute())
