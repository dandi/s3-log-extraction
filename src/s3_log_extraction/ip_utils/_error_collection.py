...
from ..config import S3_LOG_EXTRACTION_BASE_FOLDER_PATH
...
def _collect_error(message: str, error_type: str, task_id: str | None = None) -> None:
    ...
    task_id : str or None, optional
        ...
        Added as an identifying tag on the error collection file name.
    """
    errors_folder_path = S3_LOG_EXTRACTION_BASE_FOLDER_PATH / "errors"
    errors_folder_path.mkdir(exist_ok=True)

    dandi_s3_log_parser_version = importlib.metadata.version(distribution_name="s3_log_extraction")
